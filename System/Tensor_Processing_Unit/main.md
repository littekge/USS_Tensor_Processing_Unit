...
- **SPI Link:** `TPU/PROGRAMMER/TPU_SPI_LINK/SPI_Slave.v` and `TPU/PROGRAMMER/TPU_SPI_LINK/SPI_INPUT_BUFFER/SPI_Input_buffer.v` are correctly wired together by `TPU/PROGRAMMER/TPU_SPI_LINK/SPI_Interface.v` which is currently the top level module for the `TPU/PROGRAMMER/TPU_SPI_LINK/` subdirectory. The SPI input buffer has been verified to properly receive and store data from the SPI slave module.
- **VGA Debugging:** `TPU/TPU_VGA_DEBUG/debug.v` can be successfully used to display 32 bit integers as hexadecimal values on a monitor connected to the FPGA via it's VGA port.
