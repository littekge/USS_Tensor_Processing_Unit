...
- Description: Top level module for the TPU. In accordance with the module hierarchy, this module instantiates the majority of the other modules. This module also defines connection routing between its various submodules and serves as a passthrough for relevant signals (clk, rst, SPI Signals, etc.).
    - **Synchronous Control Signals:**
        - **Inputs:** 
            - ***clk***: Gobal clock (*clk*) input signal. *clk* is the global system clock for the TPU (active HIGH on positive edge). *clk* is passed to all other modules.
            - ***rst***: Gobal reset (*rst*) input signal (active LOW). *rst* is the global system reset signal, and effectively resets the entire TPU to it's default state while asserted LOW.
    - **Dual Vector Processors:** The TPU module instantiates two Vector_Processor modules, a primary processor *a* and a secondary processor *b*. Relevent connections are described below.
        - **Memory Connections:** Vector processor *a* is connected to the *a* ports of the Weight_Memory and TPU_0x1_Buffer. Likewise, vector processor *b* is connected to the *b* ports.
        - **ALU:**: Each vector processor handles one input to the ALU.
        - **ALU Synchronization:** The *element_valid* signals from the vector processors are connected by an AND gate whose output is then used as the *enable* signal for the ALU.
        - **Activator:** *a* handles input to the activator.
    ...
    - **Seperate TPU Reset:** The *program* signal is connected via an AND gate to the global reset *rst*, and the resultant signal *trst* is passed to the following modules such that either *rst* or *program* can hold them in a default state: Feeder, Systolic_Array, Controller, Vector_Processor (*a* and *b*), Activator, and ALU. This is to ensure that the TPU internals cannot modify device memory while programming occurs. Likewise, the Programmer module has full control over all device memory while the *program* signal is low.
    - **Clear Signal:** *trst* is inverted and connected to the *sclr* input to the vector buffer such that the buffer is cleared when *rst* is pulled low.
...
- Description: This module writes data to the Program_Memory, Weight_Memory, and TPU_0x1_Buffer, effectively "programming" the TPU. The module defines a finite state machine that retrieves data byte by byte from the SPI input buffer (if data is available), decodes it according to the *Functional TPU Messaging Protocol* defined in `Functional_TPU_Message_Protocol_v0.1.md`, and writes it to the correct location in memory. It also instantiates the SPI_Interface module. 
    ...
    - **State Machine Flow:**
        ...
        - Write relevent bytes (dependent on function code) to the correct location in memory.
        - Repeat the decode-write cycle until a STOP code is recieved.
        - Assert *program* HIGH when a STOP code is recieved.
...
- Description: The TPU_0x1_Buffer is a Quartus IP 2-port RAM module. It functions as the unique isolated memory required for address 0x1 in `Functional_TPU_ISA_v0.2.md`.
    - **Synchronous Control Signals:**
        - **Inputs:**
            - ***clock***: Module input clock.
            - ***wren_a***: Writes data from the *data_a* input to a location in memory defined by the *address_a* input when asserted HIGH.
...
- Description: The Controller module is the "CPU" of the TPU. Combined with the Feeder module, the two implement the traditional CPU instruction cycle (Fetch-Decode-Execute-Writeback). The Controller module defines a finite state machine that converts instructions from the Feeder into control signals and manages timing and dataflow between other modules. Valid instructions and their formats are defined in `Functional_TPU_ISA_v0.2.md`.
    - **Synchronous Control Signals:**
        ...
        - **Outputs:**
            - ***controller_idle***: Asserted HIGH when the controller is ready to recieve a new instruction.
            - ***clear***: Asserted HIGH to clear buffers between each operation.
    - **Dual Vector Processors:** The TPU has two vector processors. The controller has two sets of identical control signals to interface with each vector processor. The controller leverages both processors to perform simultanious operations whenever possible (eg. simultanious loading of systolic array input buffers or simultanious streaming of data to the two inputs of the ALU). When a simultanious operation is performed, the controller starts both vector processors at the same clock cycle to ensure synchronization. 
    - **State Machine Flow:** 
        ...
        - Assert *clear* HIGH for one clock cycle.
...
- Description: The Vector_Processor module defines a state machine that handles memory accesses, data routing to and from the Activator and Systolic Array, and proper formatting of data.
    ...
    - **ISA Complience:** The Vector_Processor module formats data written to the weight memory and 0x1 buffer in accordance with the `Functional_TPU_ISA_v0.2.md`. Likewise, it expects all data read from memory to be formatted in accordance with the ISA. As a result, the Vector_Processor handles translation of data from flattened arrays in memory to matrices when loading data to the systolic array input buffers and from matrices to flattened arrays when reading from the systolic array outputs and writing back to memory.
    ...
    - **Asynchronous Control Signal Notes:** To remain ISA complient, the Vector_Processor module has various asynchronous control signal inputs and outpus that it uses to interpret data.
        ...
        - **Memory Addressing:** The Vector_Processor module defines a *mem_source_address* and *mem_dest_address* input for reading from or writing to memory respectively. Combined with the *length*, *dim0*, and *dim1* signals, the vector processor determines the exect number of elements to read or write as well as their order.
...
- Description: The ALU module implements ELEM instructions from the `Functional_TPU_ISA_v0.2.md`.
    ...
    - **Enable Behavior:** While *enable* is asserted HIGH, every clock cycle the ALU takes two input values, performs a mathematical or logical operation on them, and writes the result to the vector buffer. As a result, the ALU can process a single value by asserting *enable* HIGH for one clock cycle.
...
- Description: The Systolic_Array module instantiates the multiply accumulate units and systolic array input buffers and wires them together, implementing an output-stationary systolic array. It also defines a state machine that loads data from the systolic array input buffers to the multiply accumulate units in the correct order.
    - **Synchronous Control Signals:**
        - **Inputs:**
            - ***clk***: Module input clock.
            - ***trst***: Module reset signal (active LOW).
